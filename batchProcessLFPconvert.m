function [] = batchProcessLFPconvert(caseID,SigNal)
...
% caseID = 'STAN_1'
mainDIR = 'F:\SleepStudy2_Aim1\VRAWLFP';
...
function [outPUTS] = fileLOAD(folderDIR)
...
switch signAl
    case 'psg'
        ...
        % Band Stop notch
        disp('Notch Filter');
        procEEG.eegNOTCH = procEEG.eegTTrawN;
        for li = 1:size(procEEG.eegTTrawN,2)
            tmpEEGraw = table2array(procEEG.eegTTrawN(:,li));
        ...
        procEEG.eegTTppLp = lowpass(procEEG.eegTTppHp,35); % Only for RC+S
        ...
        procEOG.eogNOTCH = procEOG.eogTTrawN;
        for li = 1:size(procEOG.eogTTrawN,2)
            tmpEOGraw = table2array(procEOG.eogTTrawN(:,li));
        ...
        % Band Stop notch
        disp('Notch Filter');
        procEMG.emgNOTCH = procEMG.emgTTrawN;
        for li = 1:size(procEMG.emgTTrawN,2)
            tmpEMGraw = table2array(procEMG.emgTTrawN(:,li));
        ...
        actimeVec = transpose(0:seconds(30):seconds((height(TT)-1)*30)) + INPUTS.recSTART;
